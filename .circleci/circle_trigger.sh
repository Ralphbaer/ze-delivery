...
TRIGGERS=`./.circleci/pkg-build ./.circleci/pkg-config.yml`
...
HTTP_RESPONSE=$(curl -s -u ${PERSONAL_CIRCLE_TOKEN}: -o response.txt -w "%{http_code}" -X POST --header "Content-Type: application/json" -d "$DATA" $URL)
